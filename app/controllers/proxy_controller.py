...
async def _fetch_fighter_image_url(tapology_id: str) -> str | None:
    """
    Fetches a fighter's profile page from Tapology and extracts the headshot image URL.

    Args:
        tapology_id: Fighter's Tapology ID (e.g., "mario-bautista-97827")

    Returns:
        Full URL to the fighter's headshot image, or None if not found
    """
    profile_url = f"https://www.tapology.com/fightcenter/fighters/{tapology_id}"
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        # Look for the fighter's headshot image
        # Pattern 1: profile_images in img src
        import re

        # Try to find profile_images URL
        patterns = [
            r'src="(https?://images\.tapology\.com/profile_images/[^"]+)"',
            r'src="(//images\.tapology\.com/profile_images/[^"]+)"',
            r'src="(/profile_images/[^"]+)"',
            # Letterbox images as fallback
            r'src="(https?://images\.tapology\.com/letterbox_images/[^"]+)"',
            r'src="(//images\.tapology\.com/letterbox_images/[^"]+)"',
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if url.startswith("//"):
                    ...
                    url = f"https://images.tapology.com{url}"
                return url

        return None


@router.get("/fighter-image/{tapology_id}")
async def proxy_fighter_image(tapology_id: str):
    """
    Endpoint de proxy para imágenes de perfil de peleadores.

    Este endpoint obtiene la imagen de perfil de un peleador a partir de su tapology_id.
    Primero obtiene la URL de la imagen desde el perfil del peleador en Tapology,
    luego la descarga y sirve con caché.

    Path format:
    - Input: /proxy/fighter-image/mario-bautista-97827
    - Fetches fighter profile: https://www.tapology.com/fightcenter/fighters/mario-bautista-97827
    - Extracts and proxies the headshot image

    Args:
        tapology_id: ID único del peleador en Tapology (ej: "mario-bautista-97827")

    Returns:
        Imagen del peleador con headers de cache apropiados, o 404 si no se encuentra
    """
    # Create a cache key from the tapology_id
    cache_key = f"fighter_{tapology_id}"
    cache_key_hash = hashlib.md5(cache_key.encode()).hexdigest()

    # Check memory cache first (for quick access)
    if cache_key_hash in _image_cache:
        content, content_type, etag, cached_at = _image_cache[cache_key_hash]
        if time.time() - cached_at < CACHE_TTL:
            return Response(
                content=content,
                media_type=content_type,
                headers=_build_cache_headers(etag, "HIT-FIGHTER")
            )

    # Fetch the fighter's profile page and extract image URL
    try:
        image_url = await _fetch_fighter_image_url(tapology_id)

        if not image_url:
            raise HTTPException(
                status_code=404,
                detail=f"Fighter image not found for {tapology_id}"
            )

        # Extract the path for content type detection
        path = image_url.split("images.tapology.com/")[-1] if "images.tapology.com/" in image_url else tapology_id

        # Fetch the actual image
        content, content_type = await _fetch_from_tapology(image_url, path)

        # Generate ETag
        etag = hashlib.md5(content).hexdigest()[:16]

        # Clean old cache and store
        _clean_old_cache()
        _image_cache[cache_key_hash] = (content, content_type, etag, time.time())

        return Response(
            content=content,
            media_type=content_type,
            headers=_build_cache_headers(etag, "MISS-FIGHTER")
        )

    except httpx.TimeoutException:
        raise HTTPException(status_code=504, detail="Timeout fetching fighter image")
    except httpx.RequestError as e:
        raise HTTPException(status_code=502, detail=f"Error fetching fighter image: {str(e)}")
